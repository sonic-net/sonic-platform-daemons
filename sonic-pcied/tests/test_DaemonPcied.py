...
from .mock_platform import MockChassis
...
class TestDaemonPcied(object):
    ...
    def test_run(self):
        ...
        daemon_pcied.update_psu_data = mock.MagicMock()
        daemon_pcied._update_led_color = mock.MagicMock()
        daemon_pcied.update_psu_chassis_info = mock.MagicMock()

        daemon_pcied.run()
        assert daemon_pcied.first_run is False

    def test_update_psu_data(self):
        mock_psu1 = MockPsu("PSU 1", 0, True, True)
        mock_psu2 = MockPsu("PSU 2", 1, True, True)

        pciecied.platform_chassis = MockChassis()
        daemon_pcied = pcied.DaemonPcied(SYSLOG_IDENTIFIER)
        daemon_pcied._update_single_psu_data = mock.MagicMock()
        daemon_pcied.log_warning = mock.MagicMock()

        # Test platform_chassis is None
        pcied.platform_chassis = None
        daemon_pcied.update_psu_data()
        assert daemon_pcied._update_single_psu_data.call_count == 0
        assert daemon_pcied.log_warning.call_count == 0

        # Test with mocked platform_chassis
        pcied.platform_chassis = MockChassis()
        pcied.platform_chassis._psu_list = [mock_psu1, mock_psu2]
        daemon_pcied.update_psu_data()
        assert daemon_pcied._update_single_psu_data.call_count == 2
        expected_calls = [mock.call(1, mock_psu1), mock.call(2, mock_psu2)]
        assert daemon_pcied._update_single_psu_data.mock_calls == expected_calls
        assert daemon_pcied.log_warning.call_count == 0

        daemon_pcied._update_single_psu_data.reset_mock()

        # Test _update_single_psu_data() throws exception
        daemon_pcied._update_single_psu_data.side_effect = Exception("Test message")
        daemon_pcied.update_psu_data()
        assert daemon_pcied._update_single_psu_data.call_count == 2
        expected_calls = [mock.call(1, mock_psu1), mock.call(2, mock_psu2)]
        assert daemon_pcied._update_single_psu_data.mock_calls == expected_calls
        assert daemon_pcied.log_warning.call_count == 2
        expected_calls = [mock.call("Failed to update PSU data - Test message")] * 2
        assert daemon_pcied.log_warning.mock_calls == expected_calls

    @mock.patch('pcied._wrapper_get_psu_presence', mock.MagicMock())
    @mock.patch('pcied._wrapper_get_psu_status', mock.MagicMock())
    def test_update_single_psu_data(self):
        pcied._wrapper_get_psu_presence.return_value = True
        pcied._wrapper_get_psu_status.return_value = True

        psu1 = MockPsu('PSU 1', 0, True, 'Fake Model', '12345678')
        pcied.platform_chassis = MockChassis()
        pcied.platform_chassis._psu_list.append(psu1)
