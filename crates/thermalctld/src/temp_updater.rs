...
use platform_api::{PlatformApi, ThermalInfo};
...
mod tests {
    ...
    impl platform_api::PlatformApi for FakePlatform {
        fn get_chassis_info(&mut self) -> Result<platform_api::ChassisInfo, platform_api::PlatformError> {
        ...
        fn get_thermals(&mut self) -> Result<Vec<ThermalInfo>, platform_api::PlatformError> {
            Ok(self.thermals.clone())
        }
        fn get_fan_drawers(&mut self) -> Result<Vec<platform_api::FanDrawerInfo>, platform_api::PlatformError> {
            Ok(Vec::new())
        }
        fn get_fans(&mut self) -> Result<Vec<platform_api::FanInfo>, platform_api::PlatformError> {
            Ok(Vec::new())
        }
        fn set_fan_led(&mut self, _: &str, _: platform_api::LedColor) -> Result<(), platform_api::PlatformError> {
            Ok(())
        }
    }
