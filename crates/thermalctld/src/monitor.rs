...
impl Monitor {
    ...
    pub async fn run(
        ...
        // 60 is what the base class defaults to, and what a platform whose
        // thermal_policy.json omits the key gets.
        let policy_interval = platform.tm_get_interval().ok().flatten().unwrap_or(60.0);
...
mod tests {
    ...
    use platform_api::{ChassisInfo, FanDrawerInfo, FanInfo, PlatformError, ThermalInfo};
    ...
    async fn each_cycle_updates_fans_then_temperatures_then_runs_the_policy() {
        ...
        monitor(0.0)
            .run(&mut r.platform, &m.db, r.rx.clone())
            .await;
    ...
    async fn a_shutdown_before_the_first_wait_runs_no_cycle() {
        ...
        let mut r = rig(99);
        let m = MockDb::new(false);
        monitor(0.0).run(&mut r.platform, &m.db, rx).await;
    ...
    async fn a_platform_with_no_policy_interval_gets_pythons_default() {
        ...
        impl PlatformApi for Silent {
            fn get_chassis_info(&mut self) -> Result<ChassisInfo, PlatformError> {
                self.0.get_chassis_info()
            }
            fn get_thermals(&mut self) -> Result<Vec<ThermalInfo>, PlatformError> {
                self.0.get_thermals()
            }
            fn get_fan_drawers(&mut self) -> Result<Vec<FanDrawerInfo>, PlatformError> {
                self.0.get_fan_drawers()
            }
            fn get_fans(&mut self) -> Result<Vec<FanInfo>, PlatformError> {
                self.0.get_fans()
            }
        ...
        let m = MockDb::new(false);
        monitor(0.0).run(&mut silent, &m.db, r.rx.clone()).await;
