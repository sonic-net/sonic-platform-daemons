...
/// The Python module every vendor ships its platform API in.
///
/// Named here rather than inside the bridge: which vendor's package is
/// installed is the image's business, and a bridge that imported it by
/// default could not be driven over a mock.
...
async fn main() {
    ...
    let mut platform = match Bridge::new(VENDOR_MODULE) {
        Ok(p) => p,
        Err(e) => {
    ...
    let chassis = platform.get_chassis_info().ok();
    ...
    // Start the platform's thermal manager.  Four argument-less calls rather
    // than an object: `ChassisBase.get_thermal_manager()` answers a class whose
    // methods are all @classmethod and which takes the chassis back as an
    // argument, so the facade holds the chassis and passes it. Nothing here
    // re-enters Python from Rust from Python.
    if let Err(e) = platform.tm_initialize() {
    ...
    monitor
        .run(&mut platform, &db, shutdown_rx)
        .await;
...
fn leak_thread_main(interval: Duration, is_switch_bmc: bool, shutdown: watch::Receiver<bool>) {
    ...
    leak_loop(&mut platform, &tables, interval, is_switch_bmc, shutdown);
}
...
///
/// Mirrors thermalctld:937-941: a modular chassis publishes its own slot, a
/// SmartSwitch DPU publishes its DPU id, and anything else publishes neither.
/// The base class hands back the two separately, so the choice is made here
/// rather than by a platform deciding what "slot" means.
...
fn leak_loop(
    ...
    let profiles = platform.get_leak_profiles().unwrap_or_default();
    ...
    loop {
        ...
        let sensors = platform.get_leak_sensors().unwrap_or_default();
...
mod tests {
    ...
    use platform_api::{
        ChassisInfo, FanDrawerInfo, FanInfo, LeakProfile, LeakSensorInfo, PlatformError, ThermalInfo,
    };
