...
use platform_api::{FanDirection, Threshold};
...
/// Format a position as `"1"`, `"2"`, or `"-1"` when the platform does not know.
///
/// Signed because `DeviceBase.get_position_in_parent` documents -1 for
/// unknown, which an unsigned type cannot hold.
pub fn position(p: i32) -> String {
    p.to_string()
}
...
/// What a colour is called in STATE_DB.
pub trait LedName {
