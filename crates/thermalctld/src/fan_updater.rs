...
use platform_api::{FanDrawerInfo, FanInfo, FanKind, PlatformApi};

use crate::db::{StateDb, CHASSIS_INFO_KEY};
use crate::fmt;

const LED_GREEN: platform_api::LedColor = platform_api::LedColor::Green;
const LED_RED: platform_api::LedColor = platform_api::LedColor::Red;
...
struct LedWrite {
    fan_name: String,
    drawer_name: String,
    /// One of the four colours the base class declares, not a loose string:
    /// the API takes the type, and a typo can no longer reach a platform.
    color: platform_api::LedColor,
}
...
impl FanUpdater {
    ...
    pub fn update(
        ...
        if led_writes.is_empty() {
            ...
            for write in &led_writes {
                // Two calls, not one: thermalctld:547-550 sets the fan's LED
                // and then its drawer's with the same colour, tolerating
                // either being unimplemented. Folding them into one call
                // would hide which of the two a platform refused.
                let fan_result = platform.set_fan_led(&write.fan_name, write.color);
                if write.drawer_name != fmt::NOT_AVAILABLE {
                    let _ = platform.set_fan_drawer_led(&write.drawer_name, write.color);
                }
    ...
    fn update_led_color(&self, db: &StateDb, fans: &[FanInfo], drawers: &[FanDrawerInfo], stop: &dyn Fn() -> bool) {
        for fan in fans {
            ...
            let fvs = [("led_status", fmt::led(&fan.status_led))];
        ...
        for drawer in drawers {
            ...
            let fvs = [("led_status", fmt::led(&drawer.status_led))];
            if let Err(e) = db.fan_drawer.set(&drawer.name, &fvs) {
...
mod tests {
    use super::*;
    use platform_api::{FanDirection, PlatformError};
    ...
    impl platform_api::PlatformApi for FakePlatform {
        fn get_chassis_info(&mut self) -> Result<platform_api::ChassisInfo, PlatformError> {
        ...
        fn get_thermals(&mut self) -> Result<Vec<platform_api::ThermalInfo>, PlatformError> {
            Ok(Vec::new())
        }
        ...
        fn set_fan_led(&mut self, fan: &str, color: platform_api::LedColor) -> Result<(), PlatformError> {
    ...
    fn the_led_colour_the_platform_reports_reaches_the_database() {
        ...
        let mut d = drawer("drawer1");
        d.status_led = Some(platform_api::LedColor::Green);
        let mut f = fan("fan1");
        f.status_led = Some(platform_api::LedColor::Green);
    ...
    fn a_drawer_that_cannot_name_itself_is_not_published() {
        ...
        virtual_drawer.status_led = Some(platform_api::LedColor::Green);
    ...
    fn a_fan_with_no_drawer_reports_not_available() {
        ...
        f.kind = platform_api::FanKind::Psu;
