from sonic_platform_common.sonic_thermal_control.thermal_action_base import ThermalActionBase, thermal_action
...
class ControlThermalControlAlgoAction(ThermalActionBase):
    ...
    def load_from_json(self, json_obj):
        ...
        :return:
        """
        if ControlThermalControlAlgoAction.status in json_obj:
            status_str = json_obj[ControlThermalControlAlgoAction.status].lower()
            if status_str == 'true':
                self.status = True
